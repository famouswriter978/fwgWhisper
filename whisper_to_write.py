...
def whisper_to_write(model='', device='cpu', file_in=None, waiting=True, silent=True, conversation=False):
    print('model', model, 'device', device, 'file_in', file_in, 'waiting', waiting, 'silent', silent, 'conversation', conversation)
    # Initialization
    result_ready = False
    if file_in is None:
        filepaths = None
